...
- [~] Real-time pre-trade risk checks: max order size, exposure per instrument/user, leverage validation, margin sufficiency (tick/lot/notional checks scaffolded; liquidity checks added for market orders) — partial.
- [ ] Margin call & liquidation flow (partial/full) with configurable thresholds.
- [ ] Circuit breakers/volatility halts (instrument + market-wide).
