...
function MultiLandActionsPanel() {
  ...
  const myLands = selectedLands.filter(
    (land) => land.land_id && land.owner_id === user?.user_id
  );
  const unownedLands = selectedLands.filter((land) => !land.land_id);
  const othersLands = selectedLands.filter(
    (land) => land.land_id && land.owner_id !== user?.user_id
  );
  ...
  const totalPrice = unownedLands.reduce((sum, land) => {
    const landPrice =
      land.price_base_bdt || land.base_price_bdt || land.base_price || 0;
  ...
  const handleBulkPurchase = async () => {
    ...
    if (user.balance_bdt < totalPrice) {
      toast.error(
        `Insufficient balance. Need ${totalPrice} BDT, have ${user.balance_bdt} BDT`
      );
      return;
    }
    ...
    for (let i = 0; i < unownedLands.length; i++) {
      ...
      try {
        ...
        const response = await landsAPI.claimLand({
          ...
          price_base_bdt:
            land.price_base_bdt || land.base_price_bdt || land.base_price,
        });
        ...
        console.error(
          `Failed to purchase land at (${land.x}, ${land.y}):`,
          error
        );
    ...
    if (failCount > 0) {
      ...
      toast.error(
        `Failed to purchase ${failCount} land(s):\n${errorSummary}${moreErrors}`,
        { duration: 6000 }
      );
  ...
  const handleBulkFence = async (enable) => {
    ...
    for (let i = 0; i < selectedLands.length; i++) {
      ...
      try {
        ...
        // Now fence the land
        await landsAPI.manageFence(landId, enable, enable ? "1234" : null);
    ...
    if (successCount > 0) {
      toast.success(
        `${enable ? "Enabled" : "Disabled"} fence on ${successCount} land(s)`
      );
    }
    if (failCount > 0) {
      const errorSummary = errors.slice(0, 3).join("\n");
      const moreErrors =
        errors.length > 3 ? `\n...and ${errors.length - 3} more` : "";
      toast.error(
        `Failed to fence ${failCount} land(s):\n${errorSummary}${moreErrors}`,
        { duration: 6000 }
      );
  ...
  return (
    <div className="fixed bottom-16 md:bottom-20 left-2 right-2 md:left-1/2 md:right-auto md:transform md:-translate-x-1/2 md:min-w-96 md:max-w-md z-30 bg-gray-800 rounded-lg shadow-2xl border-2 border-blue-500 p-3 md:p-4">
      <div className="flex items-center justify-between mb-3 md:mb-4">
        <div>
          ...
          <p className="text-gray-400 text-xs md:text-sm">
            {selectedLands.length} land{selectedLands.length !== 1 ? "s" : ""}{" "}
            selected
        ...
        <div className="space-y-2">
          <div className="bg-gray-700 rounded-lg p-3 mb-3">
            ...
            <p className="text-gray-400 text-xs mt-1">
              {unownedLands.length} unclaimed land
              {unownedLands.length !== 1 ? "s" : ""}
            </p>
          </div>
          <button
          ...
          >
            {processing
              ? "Processing..."
              : `Purchase via Marketplace - ${totalPrice.toLocaleString()} BDT`}
          </button>
        </div>
      )}
      ...
      {ownedLands.length > 0 && !showListingForm && (
        <div
          className={`space-y-2 ${
            unownedLands.length > 0 ? "mt-3 pt-3 border-t border-gray-600" : ""
          }`}
        >
      ...
      <div className="mt-3 pt-3 border-t border-gray-700">
        <p className="text-gray-400 text-xs text-center">
          💡 Hold{" "}
          <kbd className="px-2 py-1 bg-gray-700 rounded text-xs">Ctrl</kbd> or{" "}
          <kbd className="px-2 py-1 bg-gray-700 rounded text-xs">Cmd</kbd> and
          click to select multiple lands
        </p>
      </div>
                ...
                <div className="border-t border-gray-600 pt-2 mt-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-yellow-400 font-semibold">
                      {(user?.balance_bdt || 0).toLocaleString()} BDT
                    </span>
                  </div>
                  <div className="flex justify-between mt-1">
                    ...
                    >
                      {((user?.balance_bdt || 0) - totalPrice).toLocaleString()}{" "}
                      BDT
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-blue-900/30 border border-blue-500/50 rounded-lg p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div className="text-xs text-blue-200">
                    ...
                    <p>
                      This is a simulated payment. Funds will be deducted from
                ...
                <div className="bg-red-900/30 border border-red-500/50 rounded-lg p-4">
                  <div className="flex items-start space-x-2">
                    ...
                    <p className="text-xs text-red-200">
                      ...
                      {(totalPrice - (user?.balance_bdt || 0)).toLocaleString()}{" "}
                      more BDT.
                    </p>
                  </div>
                </div>
