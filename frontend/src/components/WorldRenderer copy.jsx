  ...
  useEffect(() => {
    ...
    chunks.forEach((chunkData, chunkId) => {
      ...
      chunkData.lands.forEach((land) => {
        ...
        const TRI_HEIGHT = (LAND_SIZE * Math.sqrt(3)) / 2;

        // --- 🔧 FIX: proper coordinate packing ---
        const offsetY = (land.x % 2) * (TRI_HEIGHT / 2);
        const baseX = land.x * (LAND_SIZE / 2);
        const baseY = land.y * TRI_HEIGHT + offsetY;

        // Alternate orientation to interlock triangles
        const isUp = (land.x + land.y) % 2 === 0;

        graphics.beginFill(color);

        if (isUp) {
          // ▲ Upward-pointing triangle
          graphics.moveTo(baseX + LAND_SIZE / 2, baseY);
          graphics.lineTo(baseX, baseY + TRI_HEIGHT);
          graphics.lineTo(baseX + LAND_SIZE, baseY + TRI_HEIGHT);
        } else {
          // ▼ Downward-pointing triangle
          graphics.moveTo(baseX, baseY);
          graphics.lineTo(baseX + LAND_SIZE, baseY);
          graphics.lineTo(baseX + LAND_SIZE / 2, baseY + TRI_HEIGHT);
        }
        ...
        if (isUp) {
          ...
          graphics.lineTo(baseX, baseY + TRI_HEIGHT);
          graphics.lineTo(baseX + LAND_SIZE, baseY + TRI_HEIGHT);
          graphics.lineTo(baseX + LAND_SIZE / 2, baseY);
        } else {
          graphics.moveTo(baseX, baseY);
          graphics.lineTo(baseX + LAND_SIZE, baseY);
          graphics.lineTo(baseX + LAND_SIZE / 2, baseY + TRI_HEIGHT);
