...
function UserProfileCard({ user, isOwnProfile = false, isAdmin = false, onUpdate = null }) {
        ...
        <div className="flex-1 min-w-0">
            ...
            {user?.is_banned && (
              <div className="flex items-center gap-2 px-3 py-1 bg-red-900 border border-red-600 rounded-lg">
                <svg className="w-4 h-4 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4v2m0 4v2M7.08 6.47A9 9 0 1120.92 17.53M7.08 6.47L5.6 5" />
                </svg>
                <span className="text-red-300 text-sm font-semibold">Banned</span>
              </div>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            {isEditing && canEdit ? (
              ...
              <p className="text-gray-300 text-sm">
                {user?.bio || <span className="text-gray-500 italic">No bio added</span>}
              </p>
            )}
            {isEditing && canEdit && (
              <p className="text-gray-500 text-xs mt-1">{editingBio.length}/500 characters</p>
            )}
          </div>
          ...
          {user?.is_banned && user?.ban_reason && (
            <div className="mb-4 p-3 bg-red-900/20 border border-red-700 rounded-lg">
              <p className="text-red-400 text-sm font-semibold mb-1">Ban Reason:</p>
