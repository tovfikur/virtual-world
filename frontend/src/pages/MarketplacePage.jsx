    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="bg-gray-800 border-b border-gray-700 p-4 md:p-6">
        ...
        <p className="text-sm md:text-base text-gray-400">
          Browse and purchase virtual land
        </p>
      </div>
        ...
        <div className="bg-gray-800 rounded-lg p-4 md:p-6 mb-4 md:mb-6 border border-gray-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
            ...
            <select
              value={filters.biome}
              onChange={(e) =>
                setFilters({ ...filters, biome: e.target.value })
              }
              className="px-4 py-2 bg-gray-700 rounded-lg border border-gray-600 focus:border-blue-500 focus:outline-none"
            >
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {listings.map((listing) => {
                ...
                const landCount =
                  listing.land_count || listing.lands?.length || 1;
                const primaryBiome =
                  listing.biomes?.[0] || listing.land_biome || "unknown";
                    ...
                    >
                      ...
                      {listing.biomes && listing.biomes.length > 1 && (
                        <span className="text-white text-xs mt-1 opacity-80">
                          +{listing.biomes.length - 1} more biome
                          {listing.biomes.length > 2 ? "s" : ""}
                        </span>
                      )}
                    </div>
                        ...
                        <div>
                          ...
                          {isParcel && bbox ? (
                            <p className="font-semibold text-sm">
                              ({bbox.min_x}, {bbox.min_y}) to ({bbox.max_x},{" "}
                              {bbox.max_y})
                            </p>
                          ) : (
                            <p className="font-semibold">
                              ({listing.land_x || listing.lands?.[0]?.x},{" "}
                              {listing.land_y || listing.lands?.[0]?.y})
                            </p>
                          )}
                        </div>
                        <span
