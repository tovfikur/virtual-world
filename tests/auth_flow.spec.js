...
test.describe("Authentication Flow", () => {
  ...
  test("Login → Refresh → Protected Endpoint", async ({ request }) => {
    ...
    console.log(
      `  Refresh token: ${
        refreshToken ? refreshToken.substring(0, 20) + "..." : "not provided"
      }`
    );
  ...
  test("Chat Unread Messages Endpoint", async ({ request }) => {
    ...
    if (response.status() === 200) {
      ...
      expect(respData).toHaveProperty(
        ["total_unread", "unread_count"].find((p) => p in respData) ||
          "total_unread"
      );
...
test.describe("Session Management", () => {
  test("Single Session Enforcement", async ({ request }) => {
    ...
    console.log(
      "✓ Single session enforcement working (old session invalidated)"
    );
  });
});
