...
test('Trace: Detailed flow of peer creation when peers available', async ({ browser }) => {
  ...
  const peerListLogs = liveDebugArray.filter(entry => 
  ...
  const peerLogs = liveDebugArray.filter(entry => entry.label.includes('[PEER]'));
