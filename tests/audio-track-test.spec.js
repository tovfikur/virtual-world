...
test("Audio: Verify audio track transmission between two simulated users", async ({
  browser,
}) => {
  ...
  const peerCreationLogs = logs.filter(
    (log) => log.includes("[PEER]") && log.includes("Creating")
  );
  ...
  const trackLogs = logs.filter(
    (log) =>
      log.includes("🎤 Adding local tracks") || log.includes("✅ Track added")
  );
