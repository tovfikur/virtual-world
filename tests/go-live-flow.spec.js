...
test("Capture full Go Live audio flow with console logs", async ({
  page,
  context,
}) => {
  ...
  const liveLogsBefore = consoleLogs.filter((log) =>
    log.includes("[live]")
  ).length;
  ...
  console.log("\n========== KEY FLOW INDICATORS ==========");
  const hasStartLive = consoleLogs.some((log) =>
    log.includes("[START] Go Live")
  );
