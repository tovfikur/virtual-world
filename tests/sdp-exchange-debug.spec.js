...
test('Debug: Check if SDP offers and answers are being exchanged', async ({ browser }) => {
  ...
  page.on('console', msg => {
    const text = msg.text();
    logs.push(text);
    if (text.includes('[OFFER]') || text.includes('[ANSWER]') || text.includes('[PEER]') || text.includes('live_peers')) {
      console.log(`>>> ${text}`);
    }
  });
  ...
  const peersMessages = logs.filter(log => log.includes('live_peers received'));
  ...
  console.log('\n=== CONCLUSION ===');
  if (peerCreationLogs.length === 0 && peersMessages.some(p => p.includes('peers: Array(0)'))) {
