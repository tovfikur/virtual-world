...
test("Test: Go Live audio should work and NOT stop", async ({ browser }) => {
  ...
  const hasGetUserMedia = consoleLogs.some((log) =>
  ...
  const hasIsLiveTrue = consoleLogs.some((log) =>
    log.includes("[STATE] isLive set to true")
  );
  const hasTeardown = consoleLogs.filter((log) =>
    log.includes("teardown")
  ).length;
  const hasStopStream = consoleLogs.filter((log) =>
    log.includes("stopLocalStream")
  ).length;
  ...
  const liveLogsAfterClick = consoleLogs.filter((log) =>
    log.includes("[live]")
  );
