...
test("Debug: Check if local tracks are being added to RTCPeerConnection", async ({
  browser,
}) => {
  ...
  page.on("console", (msg) => {
    ...
    if (
      text.includes("added local track") ||
      text.includes("about to add") ||
      text.includes("ontrack") ||
      text.includes("error adding track")
    ) {
      console.log(`>>> ${text}`);
    }
  });
  ...
  // Check specific logs
  const aboutToAddLogs = logs.filter((log) =>
    log.includes("about to add local tracks")
  );
  const addedTrackLogs = logs.filter((log) =>
    log.includes("added local track")
  );
  const errorAddingLogs = logs.filter((log) =>
    log.includes("error adding track")
  );
  const noStreamLogs = logs.filter((log) =>
    log.includes("no stream to add tracks from")
  );
  ...
  if (aboutToAddLogs.length === 0) {
    console.log(
      "❌ createPeerConnection was never called (no peers to add tracks to)"
    );
    ...
    console.log(
      "⚠️ Tracks were added but no ontrack events fired (peers may not be connected yet)"
    );
  }
