...
test('Audio transmission flow - track addition and ontrack', async ({ browser }) => {
  ...
  const addTrackLogs1 = logs1.filter(log => log.includes('added local track') || log.includes('about to add local tracks'));
  ...
  const ontrackLogs2 = logs2.filter(log => log.includes('ontrack event fired'));
  ...
  const hasRemoteStream2 = logs2.some(log => log.includes('setting remote stream'));
  ...
  if (hasTrackAdded1 && !hasOntrack2) {
    ...
    console.log('This suggests the RTCPeerConnection is not properly set up between peers.');
  }
