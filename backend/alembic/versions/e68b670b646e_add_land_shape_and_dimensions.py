...
def upgrade() -> None:
    ...
    existing_cols = {col['name'] for col in inspector.get_columns('lands')}

    # Add shape column (default to square for existing lands)
    if 'shape' not in existing_cols:
    ...
    existing_indexes = {idx['name'] for idx in inspector.get_indexes('lands')}
    if 'idx_lands_shape' not in existing_indexes:
